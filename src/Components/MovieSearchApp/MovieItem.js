...
const MovieItem = ({ data }) => {
    return (
        <div className="bg-white p-3 rounded-2xl shadow-sm flex flex-col">
            <div className="h-[297px]  mb-7">
                <img src={`https://image.tmdb.org/t/p/original${data.backdrop_path}`} alt="" 
                    className="w-full h-full object-cover rounded-lg"
                />
            </div>
            <div className="mb-[27px] px-[30px]">
                <h3 className="font-semibold text-base mb-4">{data.title}</h3>
                <p className="text-[#999] text-sm">{data.overview}</p>
            </div>
            <div className="flex items-center gap-x-3 mb-8 px-[30px] mt-auto">
